...
//    this->setFixedSize(1200, 270);
    ...
    this->trenchCoatInfo->addLayout(this->undoRedoButtonsLayout);
...
void AdminGUI::createAdminButtons() {
    ...
    this->undoButton->addAction(this->undoAction);
    this->redoButton->addAction(this->redoAction);
    ...
    connect(this->undoButton, &QPushButton::clicked, this, &AdminGUI::undoButtonClicked);
    connect(this->redoButton, &QPushButton::clicked, this, &AdminGUI::redoButtonClicked);
    ...
    this->undoRedoButtonsLayout = new QHBoxLayout;
    this->undoRedoButtonsLayout->addWidget(this->undoButton);
    this->undoRedoButtonsLayout->addWidget(this->redoButton);
}
...
void AdminGUI::addButtonClicked() {
    ...
    try{
        // the push command first executes the redo!!
        this->undoStack->push(new AddCommand(this->service, this->trenchCoatsTable, trench));
    }
...
void AdminGUI::deleteButtonClicked() {
    ...
    try{
        int position = this->service.searchForUniqueTrenchCoat(trench);
        if(position == -1)
            throw std::invalid_argument("The trench coat to be removed does not exist");
        this->undoStack->push(new DeleteCommand(this->service, this->trenchCoatsTable, trench, position));
    }
...
void AdminGUI::updateButtonClicked() {
    ...
    try{
        int position = this->service.searchForUniqueTrenchCoat(trench);
        if(position == -1)
            throw std::invalid_argument("The trench coat to be updated does not exist");
        TrenchCoat oldTrench = this->service.getTrenchCoatByPositionService(position);
        this->undoStack->push(new UpdateCommand(this->service, this->trenchCoatsTable, oldTrench, trench, position));
    }
    catch(std::exception &ex){
        QMessageBox errorMessageBox;
        errorMessageBox.setText(ex.what());
...
void AdminGUI::undoButtonClicked() {
    if(!this->undoStack->canUndo()){
        QMessageBox msg;
        msg.setText("No undos left!");
        msg.exec();
        return;
    }
    this->undoStack->undo();
...
void AdminGUI::redoButtonClicked() {
    if(!this->undoStack->canRedo()){
        QMessageBox msg;
        msg.setText("No redos left!");
        msg.exec();
        return;
    }
    this->undoStack->redo();
