...
bool FixedCapBiMap::replace(TKey key, TKey oldValue, TValue newValue)
{
	int index = 0;
	while (index < this->mapSize) {
		if (this->elements[index].first == key && this->elements[index].second == oldValue) {
