...
void testReplace() {
	...
	ValuePair one = std::pair<TValue, TValue>(1, NULL_TVALUE);
	assert(m.search(1) == one);
	ValuePair two = std::pair<TValue, TValue>(2, NULL_TVALUE);
	assert(m.replace(1, 1, 2) == true);
	assert(m.search(1) == two);
	assert(m.replace(1, 1, 2) == false);
}
